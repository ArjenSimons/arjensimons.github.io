...
try {
  ...
  const championHeaders=[['Champion','champion'],['Games','games'],['Win rate','winrate'],['Score','averageScore'],['KDA','averageKda'],['KP','averageKp'],['CS/min','csPerMinute'],['Gold/min','goldPerMinute'],['Damage/min','damagePerMinute'],['Vision/min','visionPerMinute']];
  ...
  const championHeaderRow=(sortKey='games',direction=1)=>championHeaders.map(([label,key])=>`<th aria-sort="${key===sortKey?(direction===1?(key==='champion'?'ascending':'descending'):(key==='champion'?'descending':'ascending')):'none'}"><button class="table-sort-button" data-sort="${key}">${label}<i aria-hidden="true"></i></button></th>`).join('');
  ...
  let championSortKey='games', championSortDirection=1;
  const sortChampions=(key,direction)=>{
    championSortKey=key; championSortDirection=direction;
    const sorted=[...champions].sort((a,b)=>championSorters[key](a,b)*direction||b.games-a.games||a.champion.localeCompare(b.champion));
    document.querySelector('#champion-performance-rows').innerHTML=championRows(sorted);
    document.querySelector('#champion-performance-head').innerHTML=championHeaderRow(key,direction);
  };
  document.querySelector('#champion-performance-sort').addEventListener('change',event=>sortChampions(event.target.value,1));
  document.querySelector('.player-champion-table').addEventListener('click',event=>{
    const button=event.target.closest('.table-sort-button');
    if(!button) return;
    const key=button.dataset.sort;
    document.querySelector('#champion-performance-sort').value=key;
    sortChampions(key,key===championSortKey?-championSortDirection:1);
  });
