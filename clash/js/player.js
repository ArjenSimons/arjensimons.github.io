...
try {
  ...
  const records=[]; const teammateMap=new Map(); let mvps=0, ints=0;
  tournaments.forEach(tournament=>{
    ...
    tournament.games.forEach((game,index)=>{
      const row=scoreGame(game).find(p=>p.playerId===playerId);
      if(!row) return;
      const minutes=Math.max(1,num(game.durationSeconds)/60);
      records.push({...row,result:game.result,opponent:game.opponent,date:tournament.date,tournamentId:tournament.id,tournamentName:tournament.name,gameNumber:index+1,durationSeconds:num(game.durationSeconds),damagePerMinute:num(row.damage)/minutes,goldPerMinute:num(row.gold)/minutes,csPerMinute:num(row.cs)/minutes,visionPerMinute:num(row.visionScore)/minutes,kpPercent:num(row.kp)*100});
  ...
  root.innerHTML=`
    <header class="player-hero card"><div><p class="eyebrow">Player profile</p><h1>${name}</h1><p>${role} · Most played champion: ${favoriteChampion}</p></div><div class="player-hero-rating"><strong>${average(records,'score').toFixed(1)}</strong><span>${grade(average(records,'score'))} average rating</span></div></header>
    <section class="player-summary-grid">${statCard('Games',records.length)}${statCard('Record',`${wins}-${records.length-wins}`)}${statCard('Win rate',pct(wins/records.length))}${statCard('Average KDA',average(records,'kda').toFixed(2))}${statCard('Average KP',pct(average(records,'kp')))}${statCard('MVPs / INTs',`${mvps} / ${ints}`)}${statCard('Longest win streak',longestStreak(records,'win'))}${statCard('Longest loss streak',longestStreak(records,'loss'))}</section>
